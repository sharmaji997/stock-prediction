...
def fetch_history(ticker: str, period: str = "1y") -> pd.DataFrame:
    ...
    # Guard against duplicate column names (can still happen if yfinance
    # ever returns more than one ticker's data) -- with dupes, df["Close"]
    # returns a DataFrame instead of a Series and breaks every calculation
    # downstream, so treat that as "no usable data" rather than crashing.
